...
// by `material_index` from push constants.
...
fn unpack_model(p : PushConstants) -> mat4x4<f32> {
    return mat4x4<f32>(
...
fn fs_main(in : VertexOutput) -> GBufferOutput {
    ...
    let mat_color = vec3<f32>(
        f32((push.material_index >> 0u) & 0xffu) / 255.0,
        f32((push.material_index >> 8u) & 0xffu) / 255.0,
        f32((push.material_index >> 16u) & 0xffu) / 255.0,
    );
    let roughness = 0.5;
    let metallic = 0.0;
    ...
    out.motion_depth_id = vec4<f32>(motion, in.view_depth, f32(push.instance_id));
    return out;
}
