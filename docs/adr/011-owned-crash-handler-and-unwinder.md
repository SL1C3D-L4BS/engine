# ADR-011 · Owned crash handler and unwinder

- Status: Accepted
- Date: 2026-05-18
- Phase: 0 (pre-v1.0 — contract-exempt per risk R-03)

## Summary

Frame-pointer-based unwinding, no libunwind, no heap in the signal handler, 16 MB pre-allocated crash buffer, separate crash daemon that cannot crash because it does nothing.

---
*Stub. Full Context / Decision / Rationale / Consequences to be expanded per spec Part XX.2.*
